...
Formatted, versioned legislative data for Connecticut. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for Connecticut — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/ct-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`ct-legislation`) — reads the scraper repo's output and:
...
Current settings for Connecticut:

- **State code**: `ct`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/ct-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:ct/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:ct/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:ct/sessions/`, organized by session and bill.
...
```json
{
  "HB999": {
    ...
    "vote_count": 2,
    "event_count": 0,
    "path": "country:us/state:ct/sessions/103/bills/HB999"
  }
}
```
...
- Check the scraper repo (`govbot-openstates-scrapers/ct-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
